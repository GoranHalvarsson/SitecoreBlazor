...
namespace SitecoreBlazorHosted.Client.RouterExtended
{
  internal class RouteTable
  {
    public RouteTable(RouteEntry[] routes)
    {
      ...
      {
        Console.WriteLine("17 ->" + route.Template?.TemplateText + "-" + route.Handler?.ToString());
      }
    }
    ...
    //public static RouteTable CreateNew(dynamic routesValue)
    //{
    //  var routes = new List<RouteEntry>();
    //  JArray myarray = (JArray)JObject.Parse(routesValue)["routes"];
    //  foreach (var item in myarray)
    //  {
    //    var token = item["children"];
    //    if (token != null)
    //    {
    //      var childrenArray = (JArray)token;
    //      var path = (string)item["path"];
    //      foreach (var child in childrenArray)
    //      {
    //        var childPath = (string)child["path"];
    //        routes.Add(new RouteEntry(TemplateParser.ParseTemplate($"{path}{childPath}"), Type.GetType($"{(string)child["component"]}")));
    //      }
    //    }
    //    else
    //    {
    //      routes.Add(new RouteEntry(TemplateParser.ParseTemplate((string)item["path"]), Type.GetType($"{(string)item["component"]}")));
    //    }
    //  }
    ...
    public static RouteTable CreateNew(string routesValue)
    {
      var routes = new List<RouteEntry>();
      //Console.WriteLine(routesValue);
      //Models.RouterDataRoot routesData = JsonUtil.Deserialize<Models.RouterDataRoot>(routesValue);

      Models.RouterDataRoot routesData = new Models.RouterDataRoot
      {
        Routes = new List<Models.RouterData>()
        {
          new Models.RouterData()
          {
            Path = "/{Language}",
            Page = "SitecoreBlazorHosted.Client.Shared.MasterBlaster, SitecoreBlazorHosted.Client"
          },
          new Models.RouterData()
          {
            Path = "/{Language}/habitat-stuff",
            Page = "SitecoreBlazorHosted.Client.Shared.MasterBlaster, SitecoreBlazorHosted.Client",
            Children = new List<Models.RouterData>()
            {
              new Models.RouterData()
              {
                Path = "/teasers",
    ...
    public static RouteTable Create(IEnumerable<Type> types)
    {
      ...
      foreach (var type in types)
      {
        ...
        {
          Console.WriteLine("496 ->" + routeAttribute.Template);
